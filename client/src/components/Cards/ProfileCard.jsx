import { useState } from "react"
import ProfileContentCard from "../Cards/ProfileContentCard.jsx"
import EditProfileCard from "../Cards/EditProfileCard.jsx"
import RentInfoCard from "../Cards/RentInfoCard.jsx"

import useFetch from "../../hooks/useFetch.js"

export function ProfileCard() {
  const { data, loading, error } = useFetch("/member/6336a288006cea151797d6f0/")
  const src =
    "data:image/jpeg;base64,"
  const buttonString =
    "w-full h-10 px-10 lg:ml-0 m-2 text-white text-sm text-center transition-colors duration-150 bg-blue-700 rounded-lg focus:shadow-outline hover:bg-blue-800"
  const [profileShown, setProfileIsShown] = useState(true)
  const [rentShown, setRentIsShown] = useState(false)
  const [editShown, setEditIsShown] = useState(false)
  const [popUpShown, setPopUpIsShown] = useState(false)
  const rentClick = (event) => {
    setRentIsShown((current) => !current)
    setProfileIsShown(false)
    setEditIsShown(false)
  }
  const profileClick = (event) => {
    setRentIsShown(false)
    setProfileIsShown((current) => !current)
    setEditIsShown(false)
  }

  const editClick = (event) => {
    setRentIsShown(false)
    setProfileIsShown(false)
    setEditIsShown((current) => !current)
  }

  return (
    <div className="App flex flex-col lg:h-screen lg:flex-row justify-center items-center bgdefault gap-x-4 pt-2">
      <div className="bg-white rounded-md mt-4 lg:mt-0 my-4 mx-10 lg:p-8 flex lg:flex-col items-center">
        <div className="hidden lg:flex">
          <img src={src} alt="avatar" className="h-32 rounded-full" />
        </div>
        <div id="profile-buttons" className="flex lg:flex-col">
          <button class={buttonString} onClick={profileClick}>
            Profile
          </button>
          <button class={buttonString} onClick={rentClick}>
            Rent
          </button>
          <button class={buttonString} onClick={editClick}>
            Edit
          </button>
        </div>
      </div>

      {profileShown && <ProfileContentCard />}
      {rentShown && <RentInfoCard />}
      {editShown && <EditProfileCard />}
    </div>
  )
}

export default ProfileCard
