...
const ProfileContentCard = () => {
  ...
  //     "/member/6336a288006cea151797d6f0/"
  //   );
  ...
  return (
    <div className="mx-8 mt-2 lg:mt-36 mb-52 w-9/12 md:w-5/6 lg:w-[986px] bg-white rounded-md p-12 flex flex-col">
      {loading ? (
        "Loading Please Wait "
      ) : (
        <>
          <div
            id="Hello"
            className="text-center text-xl lg:text-4xl font-extrabold text-blue-700 text-left mb-4"
          >
            Hello, <span className="text-yellow-500">{userData.name}</span>{" "}
          </div>
          <div>
            <img
              src={src}
          ...
          <div
            id="Name"
            className="text-xl lg:text-2xl font-bold text-center mb-4"
          >
            {userData.isAdmin && <p>Admin</p>}
            {!userData.isAdmin && <p>Member</p>}
          </div>

          <div id="memberStatus" className="text-xl text-center"></div>
          <div className="flex flex-col lg:grid lg:grid-cols-3">
            <div className="px-1 py-2 font-semibold">Email :</div>
            <div className="px-1 py-2">{userData.email}</div>
          </div>

          <div className="flex flex-col lg:grid lg:grid-cols-3">
            <div className="px-1 py-2 font-semibold">No Handphone :</div>
            <div className="px-1 py-2">{userData.phoneNumber}</div>
          </div>

          <div className="flex flex-col lg:grid lg:grid-cols-3">
            <div className="px-1 py-2 font-semibold">Booking Status :</div>
            {userData.bookingStatus && (
              <div className="px-1 py-2">Currently Booking</div>
            )}
            {!userData.bookingStatus && (
              <div className="px-1 py-2">Currently Not Booking</div>
            )}
          </div>
        </>
      )}
