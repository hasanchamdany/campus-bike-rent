...
const mongodb_ace_autocompleter_1 = require("mongodb-ace-autocompleter");
exports.BASE_COMPLETIONS = mongodb_ace_autocompleter_1.EXPRESSION_OPERATORS.concat(mongodb_ace_autocompleter_1.CONVERSION_OPERATORS.concat(mongodb_ace_autocompleter_1.BSON_TYPES.concat(mongodb_ace_autocompleter_1.STAGE_OPERATORS)));
exports.MATCH_COMPLETIONS = mongodb_ace_autocompleter_1.QUERY_OPERATORS.concat(mongodb_ace_autocompleter_1.BSON_TYPES);
...
async function completer(params, line) {
    ...
    if (splitLine.length <= 1) {
        ...
        if (splitLine[2].includes('([') || splitLine[2].includes('({')) {
            let expressions;
            if (splitLine[2].match(/\baggregate\b/)) {
                expressions = exports.BASE_COMPLETIONS.concat(getStageAccumulators(params, elToComplete));
            }
            else {
...
function isAcceptable(params, entry, versionKey) {
    ...
    const isAcceptableEnvironment = !entry.env ||
        !connectionInfo ||
        (connectionInfo.is_data_federation ? entry.env.includes(mongodb_ace_autocompleter_1.ADL) :
            connectionInfo.is_atlas ? entry.env.includes(mongodb_ace_autocompleter_1.ATLAS) :
                entry.env.includes(mongodb_ace_autocompleter_1.ON_PREM));
...
function getStageAccumulators(params, stage) {
    ...
    if (stage.includes(PROJECT)) {
        return mongodb_ace_autocompleter_1.ACCUMULATORS.filter((acc) => {
            return isAcceptable(params, acc, 'projectVersion');
        });
    }
    else if (stage.includes(GROUP)) {
        return mongodb_ace_autocompleter_1.ACCUMULATORS;
    }
}
